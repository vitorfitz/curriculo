...
let callsDM,callsQFE,callsnm,callsq;
function melhorPlay(o0,nrest,qrest,aspa,aspb){
  ...
  function add2transp(o1,mov,rest,alpha,beta){
    ...
    function evalTurn(i){
      if(provs>=0){
        for(let j=0; j<provaveis[provs].length; j++){
          if(movsIguais(os[i].mov,provaveis[provs][j])){
            return 26-j;
          }
        }
      }
  ...
  function negamax(o,rest,plays,alpha,beta){
    callsnm++;
    if(rest==0){
    ...
    if(objTrans){
      if(hashDesse==objTrans.hash){
        ...
        else if(objTrans.mov){
          jogadaPrioritaria=objTrans.mov;
        }
      }
    }

    let max=cor==0? -Infinity: Infinity,i,imax,melhorou=false,parou=false,ab=cor==0? alpha: beta;

    if(jogadaPrioritaria){
      let posJP=mexida(o,jogadaPrioritaria);
      max=negamax(posJP,rest-1,plays+1,alpha,beta);
      imax=0;
      os.push(posJP);
    ...
    if(!parou){
      ...
      /*if(novas.reiComido){
        ...
        add2transp(o,novas[novas.length-1].mov,99,max,max);
        return max;
      }*/
      ...
      if(cor==0){
        for(i=numVelhas; i<os.length; i++){
          const res=negamax(os[i],rest-1,plays+1,ab,beta);
          if(res>max){
            max=res;
            imax=i;
          }
        ...
        let b=beta;
        for(i=numVelhas; i<os.length; i++){
          const res=negamax(os[i],rest-1,plays+1,alpha,ab);
          if(res<max){
            max=res;
            imax=i;
          }
    ...
    let mov=melhorou? os[imax].mov: null;
    if(first) melhor=mov;
    else if(parou){
      novaProvavel(plays,os[imax]);
    }
    if(max<=alpha){
      add2transp(o,mov,rest,-Infinity,max);
    }
    else if(max<beta){
      add2transp(o,mov,rest,max,max);
    }
    else{
      add2transp(o,mov,rest,max,Infinity);
    }
    return max;
  }

  return{
    eval:negamax(o0,nrest,0,aspa,aspb),
    mov:melhor
  }
}
