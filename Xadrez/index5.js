...
const multPassado=[0,1.2,1.2,1.3,1.55,2.3,1,0]; // Todo
...
let durTrans=250,esperaBot=500;
...
function saveState(){
  ...
  if(plays>=playQueComecou){
    ...
    let tempos2=[...tempos];
    const oc=+!vez;
    tempos2[oc]=Math.round(tempos2[oc]-performance.now()+comecoVez+incrs[oc]);
    o.tempos=tempos2;
    console.log(tempos2);
...
function voltarPara(p){
  limpar(imgs);
  for(let i=0,n=tilesJogPassada.length; i<n; i++){
    tilesJogPassada[i].classList.remove("ultima");
  }
  for(let i=0; i<8; i++){
    for(let j=0; j<8; j++){
      if(p.pos[i][j]!=null){
...
let timeoutTempo,intervalTempo;
let comecoVez;
function eVezDoBot(){
  const oc=+!vez;
  if(plays-playQueComecou>=3){
    clearInterval(intervalTempo);
    clearTimeout(timeoutTempo);
    tempos[oc]=Math.round(tempos[oc]-performance.now()+comecoVez+incrs[oc]);
  }
  if(plays-playQueComecou>=2){
    comecoVez=performance.now();
  }
  if(plays-playQueComecou>=3){
    updateClock(oc);
  }
  if(!xequeMate && btnEmpate.disabled && tempos[oc]>0 && ((contraOBot & 1) && vez==0 || (contraOBot & 2) && vez==1)){
    setTimeout(botplay,esperaBot);
  }
  else{
    if(plays-playQueComecou>=2){
      timeoutTempo=setTimeout(function () {
        updateClock(vez);
...
function highlightMove(){
  for(let i=0; i<tilesJogPassada.length; i++){
    tilesJogPassada[i].classList.add("ultima");
  }
}
...
function liberarMov(i,j,f){
  ...
  tds[i][j].event=function () {
    f();
    tilesJogPassada.push(tds[i][j]);
    highlightMove();
  };
...
const divTempos=document.querySelector("#tempos");
...
function doisDigitos(x){
  return (x<10? "0": "")+x;
}
function updateClock(cor){
  let s=Math.ceil((tempos[cor]-(performance.now()-comecoVez))/1000);
  ms[cor].innerHTML=doisDigitos(Math.floor(s/60));
  ss[cor].innerHTML=doisDigitos(s%60);
  if(s<=0){
    clearTimeout(timeoutTempo);
    clearTimeout(intervalTempo);
  }
}
...
function iniciarSelecao(){
  ...
  divTempos.classList.add("oculto");
...
function terminarSelecao(){
  ...
  divTempos.classList.remove("oculto");
  vez=0;
  jogadas=0;
  tempos=[...temposInic];
  resetClocks();
...
function undo(){
  if(plays>0){
    ...
    if(plays<=playQueComecou){
      ...
      tempos=[...boardStates[plays].tempos];
      resetClocks();
    }
  }
}
...
let personas=[
  ...
  {
    nome:"Stockshit invertido",
    desc:`Joga primeiro e pensa depois. As consequências disso são imprevisíveis.`,
    elo:1400,
    calc:25,
    estr:75,
    estilo:5
...
function penalidadeSemRoque(rs,pos,cor){
  ...
  for(let i=0,c=1; i<2; i++,c=6){
    if(!rs[i]){
      ...
      pts[i]=avaliarEstrutura(c,l,pos,cor,false)/4;
    }
  }
  if(estilo==2){
    pts[0]*=1.333;
...
function quaoFerradoEstou(pos,roques,cReis,lReis){
  ...
  const estagioRei=calcEstagio(material);
  for(let i=0; i<8; i++){
    for(let j=0; j<8; j++){
      if(pos[i][j]!=null && pos[i][j].id!=4){
        let cor=pos[i][j].cor;
  ...
  if(estilo!=1){
    for(let cor=0,x=1; cor<=1; cor++,x=-1){
      ...
      res+=((posDoRei[j2][cReis[cor]]-avaliarEstrutura(cReis[cor],lReis[cor],pos,cor,true)*0.75)*estagioRei*penalidadeSemRoque(roques[cor],pos,cor)*function () {
        switch(estilo){
          case 2:
          return 1.5;
          default:
      ...
      }()+posDoReiFim[j2][cReis[cor]]*(1-estagioRei))*x;
    }
  }
...
function melhorPlay(o0,nrest,qrest,aspa,aspb,tempoMax,proibidas=[]){
  ...
  function negamax(o,rest,plays,alpha,beta,nmp=true){
    ...
    if(!first && repetiunvezes(cor,0)){
      if(estilo==5) add2transp(o,null,999999,0,0,nmp);
      return 0;
    }
    ...
    let os=[],hashMove=null;
    if(objTrans && !(first && proibidas.length>0)){
      if(o.hash==objTrans.hash){
        if(objTrans.rest>=rest && !(first && objTrans.mov==null)){
          ...
          if(objTrans.alpha>=beta){
            if(first) melhor=objTrans.mov;
            if(first) console.warn("pegou a",objTrans);
            return objTrans.alpha;
          }
          if(objTrans.beta<=alpha){
            if(first) melhor=objTrans.mov;
            if(first) console.warn("pegou b",objTrans);
            return objTrans.beta;
          }
          if(!first){
            nmp=objTrans.nmp;
    ...
    if(!xeque && nmp && numPecas[cor]>0 && isFinite(ba)){
      ...
      if(cor==0){
        if(res>=ba){
          add2transp(o,null,rest,ba,Infinity,nmp);
          return res;
        }
      }
      else{
        if(res<=ba){
          add2transp(o,null,rest,-Infinity,ba,nmp);
          return res;
        }
      }
    }
    ...
    let i,imax=0,melhorou=false,parou=false;

    if(hashMove){
      mexida(o,hashMove);
      max=negamax(o,rest-1,plays+1,alpha,beta);
      desmexida(o,hashMove);
    ...
    if(!parou){
      ...
      if(novas.reiComido){
        ...
        add2transp(o,melhor,999999,max,max,nmp);
        return max;
      }
      ...
      if(cor==0){
        for(let i=numVelhas; i<os.length; i++){
          ...
          if(rest>=2 && !xeque && os[i].eval<=10){
            res=negamax(o,rest-2,plays+1,alpha,beta);
            if(res>alpha && res!=abortar) {
              res=negamax(o,rest-1,plays+1,alpha,beta);
              m++;
            }
          }
          else{
    ...
    if(!xeque && max==1000001000*(cor==0? -1: 1) && !first){
      add2transp(o,null,999999,0,0,nmp);
      return 0;
    }
    else if(Math.abs(max)>1000000000){
      if(max>0){
        max--;
      }
      else{
...
let temposInic=[600000,600000],tempos;
let incrs=[6000,6000];
let tempoMin=5000,tempoMax=5*tempoMin;
let n;
let contraOBot=2; // 0 = sem bot, 1 = bot branco, 2 = bot preto, 3 = das 2 cores
let lastEval=0.000000001;
...
function asp(o,tempoMin,tempoMax,lastEval,pbd=[]){
  let res=null;
  let mate=Math.abs(lastEval)>=1000000000 || lastEval==0;
  n=performance.now();
  let i=1;
  a:for(; performance.now()-n<tempoMin; i++){
    ...
    while(true){
      ...
      res2=melhorPlay(o,i,6,aspa,aspb,i==1? Infinity: tempoMax,pbd);
...
function asprest(o,rest,tempoMax,lastEval,pbd=[]){
  let res=null;
  let mate=Math.abs(lastEval)>=1000000000 || lastEval==0;
  n=performance.now();
  a:for(let i=1; i<=rest; i++){
    let inta=intBase,intb=intBase;
    let res2;
    while(true){
      let aspa,aspb;
...
function multipv(o0,vacilo,tempoMin,tempoMax){
  let pbd=[];
  let os=playArray(o0,false,-1,[],xeque,true);
  let best=asp(o0,tempoMin,tempoMax,0.000000001);
  let pre,res=best;
  let i=0;
  do{
    ...
    pbd.push(res.mov);
    pre=res;
    res=asprest(o0,best.rest,tempoMax,0,pbd);
    i++;
  }while(i<=5 && (vez==0? best.eval-res.eval<vacilo: (res.eval-best.eval<vacilo)));

  function d(x){
    return Math.abs(x-best.eval+vacilo*(vez==0? 1: -1));
  }
  const d1=d(res.eval);
  const d2=d(pre.eval);
  console.log(best,pre,res);
  if(d1<d2){
    return res;
  }
  else{
...
function botplay(){
  if(plays-playQueComecou<=1){
    tempoMin=5000;
    tempoMax=15000;
  }
  else{
    tempoMin=Math.min((tempos[vez]-esperaBot)/120+incrs[vez]/3,5000);
    tempoMax=Math.min(5*tempoMin,tempos[vez]-esperaBot);
  }
  ...
  let res;
  if(estilo==4){
    res=multipv(o,3,tempoMin/5,tempoMax/5);
  }
  else if(estilo==5){
    res=asprest(o,1,tempoMax,lastEval);
    mexida(o,res.mov);
    const res2=asp(o,tempoMin,tempoMax,lastEval);
    desmexida(o,res.mov);
    lastEval=res2.eval;
  }
  else{
  ...
  try{
    for(let i=0; i<15; i++){
      ...
      if(i==0){
        console.log("primeira coisa e",objTrans);
      }
    ...
    console.warn("gambiarra");
  }
  console.log(seq);

  const mov=res.mov;
  mexida(o,mov,false);
  ultimaJogada=toNotation(casas,o.pos,mov,vez);
  roques=o.roques;
  ...
  jogada();
  tilesJogPassada=[tds[mov.cOrig==8? 4: mov.cOrig][mov.lOrig],tds[mov.cDest][mov.lDest]];
  highlightMove();
