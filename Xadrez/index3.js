...
const prototiposPecas=[
  {
    ...
    fnValor:function (pos,i,j) {
      return pos[i][j].mob*0.5*test+this.preco;
    }
  },
  {
    imgs:["wn.png","bn.png"],
    ...
    preco:95,
    fnValor:function (pos,i,j) {
      return pos[i][j].mob*1*test+this.preco;
    }
  },
  {
    imgs:["wb.png","bb.png"],
  ...
  },
  {
    ...
    fnValor:function (pos,i,j,estagio=0.5,cor=pos[i][j].cor) {
      ...
      return this.preco+mp[j2][i]*estagio+mpFim[j2][i]*(1-estagio);
    }
  },
  {
    imgs:["wa.png","ba.png"],
...
function jogada(){
  ...
  const xequeMate=numMovs==0 && podeEnPassant.length==0;
  if(xequeMate){
    if(xeque){
...
function calcEstagio(mat){
  // Quadrática com pontos (0, 0); (627, 0.5); (2000, 1)
...
function penalidadeSemRoque(rs){
  if(rs[0]){
    if(rs[1]){
      return 1;
    }
...
function quaoFerradoEstou(pos,roques,cReis,lReis){
  ...
  for(let i=0; i<8; i++){
    for(let j=0; j<8; j++){
      if(pos[i][j]!=null){
        res+=prototiposPecas[pos[i][j].id].fnValor(pos,i,j,estagio)*(pos[i][j].cor==0? 1: -1);
      }
    }
  }
  ...
  for(let cor=0,x=1; cor<=1; cor++,x=-1){
    let estrutura=0;
    for(let i=cReis[cor]-1; i<=cReis[cor]+1; i++){
      if(i>=0 && i<=7){
        let temPeao=false;
        for(let j=lReis[cor]; j!=lReis[cor]+3*x; j+=x){
          if(j>=0 && j<=7){
            if(pos[i][j]!=null && pos[i][j].id==5){
              temPeao=true;
              break;
            }
          }
    ...
    res+=((posDoRei[j2][cReis[cor]]+estrutura)*estagio*penalidadeSemRoque(roques[cor])+posDoReiFim[j2][cReis[cor]]*(1-estagio))*x;
  }
...
function mexida(o,mov,mudar=true){
  ...
  if(mudar){
    ...
    /*if(cp.id==5 && dest!=null){
      ...
      if(peoesPorColuna[cor][mov.cDest]>1){
        dobrados[cor]++;
      }
      if(peoesPorColuna[cor][mov.cOrig]>=1){
        dobrados[cor]--;
      }
    }
    if(dest!=null && dest.id==5){
      const oc=+!cor;
      peoesPorColuna[oc][mov.cDest]--;
      if(peoesPorColuna[oc][mov.cDest]>=1){
        dobrados[oc]--;
      }
    }*/
  }
...
let callsDM,callsQFE,callsnm,callsq,m;
function melhorPlay(o0,nrest,qrest,aspa,aspb){
  ...
  // Teste empate        rnbq1rk1/ppp2p2/2np3p/2b1p2Q/2B1P3/P2P4/1PPN1PPP/RN2K2R w KQ - 0 1
  ...
  // Posição que trava   r3kbnr/pp2p2p/2n1bp2/2q1P1p1/3p1P2/3B1QP1/PP1PN2P/RNBK3R w kq - 0
  ...
  function playArray(o,qui,provs0,jp,xeque){
    ...
    function evalTurn(i){
      ...
      let ret=dest? prototiposPecas[dest.id]./*fnValor(pos,os[i].cDest,os[i].lDest)*/preco: 0;
      /*let p=pos[os[i].cOrig==8? 4: os[i].cOrig][os[i].lOrig];
      if(p.id==5){
        let res=prototiposPecas[os[i].id].fnValor(pos,os[i].cDest,os[i].lDest,cor)-prototiposPecas[5].fnValor(pos,os[i].cOrig,os[i].lOrig,cor);
        ret+=res;
      }*/
      return ret;
    }
  ...
  function quiesce(o,rest,alpha,beta){
    ...
    let os=playArray(o,true,-1,[],false);
    if(os.length==0){
      return quaoFerradoEstou(o.pos,o.roques,o.cReis,o.lReis);
    }
    if(os.reiComido){
  ...
  function negamax(o,rest,plays,alpha,beta,nmp=true){
    ...
    if(!parou){
      const novas=playArray(o,false,plays,os,xeque);
      if(novas.reiComido){
      ...
      if(cor==0){
        for(let i=numVelhas; i<os.length; i++){
          /*if(rest<=1 && !xeque && o.pos[os[i].cDest][os[i].lDest]==null && alpha!=-Infinity){
            const ev=quaoFerradoEstou(o.pos,o.roques,o.cReis,o.lReis);
            if(ev+60<alpha){
              continue;
            }
          }*/
        ...
        for(let i=numVelhas; i<os.length; i++){
          /*if(rest<=1 && !xeque && o.pos[os[i].cDest][os[i].lDest]==null && beta!=Infinity){
            const ev=quaoFerradoEstou(o.pos,o.roques,o.cReis,o.lReis);
            if(ev-60>beta){
              continue;
            }
...
function toNotation(p0,p,mov,cor){
  if(mov.cOrig==casaOrigRoque){
    ...
    if(p[mov.cDest][mov.lDest].id!=p0[mov.cOrig][mov.lOrig].id){
      ...
      ultimaJogada+=colunaParaLetra(mov.cDest)+(mov.lDest+1)+"="+imgPeca(mov.id,cor,true);
      return ultimaJogada;
    }
    else{
...
let tempoMin=100,tempoMax=50000;
let lastEval=0;
const intBase=3.2,mult=4;
